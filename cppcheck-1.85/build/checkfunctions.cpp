...
// pattern: asm|sizeof|catch
static bool match19(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("asm")) || (tok->str()==MatchCompiler::makeConstString("sizeof")) || (tok->str()==MatchCompiler::makeConstString("catch"))))
        return false;
    return true;
}
/*
...
void CheckFunctions::checkProhibitedFunctions()
{
    ...
    for (const Scope *scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (checkAlloca && match2(tok) && (!tok->function() || tok->function()->nestedIn->type == Scope::eGlobal)) {
                if (mTokenizer->isC()) {
...
void CheckFunctions::checkIgnoredReturnValue()
{
    ...
    for (const Scope *scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (match4(tok))
                tok = tok->linkAt(1);
            else if (match5(tok) && tok->link())
                tok = tok->link();

            if (tok->varId() || !match1(tok))
                continue;
            ...
            if ((!tok->function() || !match6(tok->function()->retDef)) &&
...
void CheckFunctions::checkMathFunctions()
{
    ...
    for (const Scope *scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (styleC99) {
                if (match12(tok) && Tokenizer::isOneNumber(tok->str()) && tok->next()->astOperand2() == tok->tokAt(3)) {
                    mathfunctionCallWarning(tok, "1 - erf(x)", "erfc(x)");
                } else if (match14(tok) && match13(tok->linkAt(1)) && Tokenizer::isOneNumber(tok->linkAt(1)->strAt(2)) && tok->linkAt(1)->next()->astOperand1() == tok->next()) {
                    mathfunctionCallWarning(tok, "exp(x) - 1", "expm1(x)");
                } else if (match15(tok) && tok->next()->astOperand2()) {
...
//     </arg>
    ...
    for (const Scope *scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            if (match16(tok) && (numberOfArguments(tok)==3)) {
                const std::vector<const Token *> &arguments = getArguments(tok);
                if (WRONG_DATA(arguments.size() != 3U, tok))
                    continue;
                const Token* lastParamTok = arguments[2];
                if (lastParamTok->str() == MatchCompiler::makeConstString("0"))
...
//     </arg>
    ...
    for (const Scope *scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok && (tok != scope->bodyEnd); tok = tok->next()) {
            ...
            if (match18(secondParamTok) && MathLib::isNullValue(secondParamTok->str()))
                continue;
