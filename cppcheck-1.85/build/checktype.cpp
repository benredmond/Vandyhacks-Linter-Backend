...
void CheckType::checkTooBigBitwiseShift()
{
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        // C++ and macro: OUT(x<<y)
        if (mTokenizer->isCPP() && match1(tok) && match2(tok->linkAt(2)) && tok->next()->isUpperCaseName() && !tok->next()->function())
            tok = tok->linkAt(2);
...
void CheckType::checkIntegerOverflow()
{
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        ...
        if (tok->str() == MatchCompiler::makeConstString("<<") && value->intvalue > 0 && value->intvalue < (((MathLib::bigint)1) << bits))
            continue;
...
void CheckType::checkLongCast()
{
    ...
    // Assignments..
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        if (tok->str() != MatchCompiler::makeConstString("=") || !match5(tok->astOperand2()))
            continue;
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        ...
        bool islong = false;
        while (match6(def)) {
            if (def->str() == MatchCompiler::makeConstString("long") && def->originalName().empty()) {
                islong = true;
                break;
            }
            def = def->previous();
