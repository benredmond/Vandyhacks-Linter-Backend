...
// pattern: previous ( ) . next|tokAt|strAt|linkAt (
static bool match8(const Token* tok) {
    ...
    if (!tok || !(tok->str()==MatchCompiler::makeConstString(".")))
        return false;
    tok = tok->next();
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("next")) || (tok->str()==MatchCompiler::makeConstString("tokAt")) || (tok->str()==MatchCompiler::makeConstString("strAt")) || (tok->str()==MatchCompiler::makeConstString("linkAt"))))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("(")))
        return false;
...
// pattern: next ( ) . previous|tokAt|strAt|linkAt (
static bool match9(const Token* tok) {
    ...
    if (!tok || !(tok->str()==MatchCompiler::makeConstString(".")))
        return false;
    tok = tok->next();
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("previous")) || (tok->str()==MatchCompiler::makeConstString("tokAt")) || (tok->str()==MatchCompiler::makeConstString("strAt")) || (tok->str()==MatchCompiler::makeConstString("linkAt"))))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("(")))
        return false;
...
// pattern: next|previous ( ) . next|previous ( ) . next|previous|linkAt|strAt|link|str (
static bool match12(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("next")) || (tok->str()==MatchCompiler::makeConstString("previous"))))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("(")))
        return false;
    ...
    if (!tok || !(tok->str()==MatchCompiler::makeConstString(".")))
        return false;
    tok = tok->next();
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("next")) || (tok->str()==MatchCompiler::makeConstString("previous")) || (tok->str()==MatchCompiler::makeConstString("linkAt")) || (tok->str()==MatchCompiler::makeConstString("strAt")) || (tok->str()==MatchCompiler::makeConstString("link")) || (tok->str()==MatchCompiler::makeConstString("str"))))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("(")))
        return false;
...
static bool match14(const Token* tok) {
    if (!tok || !(tok->str()==MatchCompiler::makeConstString(".")))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("emplace")))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("(")))
        return false;
...
static bool match15(const Token* tok) {
    if (!tok || !(tok->str()==MatchCompiler::makeConstString(".")))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("back")))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("(")))
        return false;
...
static bool match16(const Token* tok) {
    ...
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("::")))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("string")))
        return false;
    return true;
}
/*
    ...
    for (std::size_t i = 0; i < symbolDatabase->functionScopes.size(); ++i) {
        ...
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (match8(tok) || match9(tok) ||
                (match11(tok) && match10(tok->linkAt(1)))) {
                ...
                if ((func2 == MatchCompiler::makeConstString("previous") || func2 == MatchCompiler::makeConstString("next") || func2 == MatchCompiler::makeConstString("str") || func2 == MatchCompiler::makeConstString("link")) && tok->strAt(10) != MatchCompiler::makeConstString(")"))
                    continue;
...
void CheckInternal::checkStlUsage()
{
    ...
    for (const Scope *scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            //if (match15(tok) && tok->astOperand1() && tok->astOperand1()->valueType() && tok->astOperand1()->valueType()->container && match16(tok->astOperand1()->valueType()->container))
