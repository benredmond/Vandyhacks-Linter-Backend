...
static int multiComparePercent(const Token *tok, const char*& haystack, unsigned int varid)
{
    ...
    case '|':
        //simple '%' character
        haystack += 1;
        if (tok->isArithmeticalOp() && tok->str() == MatchCompiler::makeConstString("%"))
            return 1;
        break;
    case 'v':
        if (haystack[3] == '%') { // %var%
...
std::string Token::expressionString() const
{
    ...
    while (start->astOperand1() &&
           (start->astOperand2() || !start->isUnaryPreOp() || match11(start) || start->str() == MatchCompiler::makeConstString("{")))
        start = start->astOperand1();
    const Token *end = top;
    while (end->astOperand1() && (end->astOperand2() || end->isUnaryPreOp())) {
        if (match12(end) &&
            !(match13(end) && !end->astOperand2())) {
            end = end->link();
            break;
        }
...
const Token *Token::getValueTokenDeadPointer() const
{
    ...
    for (it = values().begin(); it != values().end(); ++it) {
        ...
        if (!it->isTokValue() || (it->tokvalue && it->tokvalue->str() != MatchCompiler::makeConstString("&")))
            continue;
        // Get variable
