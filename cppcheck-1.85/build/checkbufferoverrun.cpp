...
// pattern: if|for|switch|while (
static bool match8(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("if")) || (tok->str()==MatchCompiler::makeConstString("for")) || (tok->str()==MatchCompiler::makeConstString("switch")) || (tok->str()==MatchCompiler::makeConstString("while"))))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("(")))
        return false;
...
static bool match11(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString(";")) || (tok->str()==MatchCompiler::makeConstString("{")) || (tok->str()==MatchCompiler::makeConstString("}")) || tok->isOp()))
        return false;
    tok = tok->next();
    if (!tok || !tok->isName())
        return false;
...
// pattern: %varid% = new|malloc|realloc
static bool match19(const Token* tok, const unsigned int varid) {
    ...
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("=")))
        return false;
    tok = tok->next();
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("new")) || (tok->str()==MatchCompiler::makeConstString("malloc")) || (tok->str()==MatchCompiler::makeConstString("realloc"))))
        return false;
    return true;
}
// pattern: [;{}] %varid% = %any%
...
// pattern: strcpy|strcat ( %varid% , %str%|%var% )
static bool match27(const Token* tok, const unsigned int varid) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("strcpy")) || (tok->str()==MatchCompiler::makeConstString("strcat"))))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("(")))
        return false;
    ...
    if (!tok || !(tok->str()==MatchCompiler::makeConstString(",")))
        return false;
    tok = tok->next();
    if (!tok || !((tok->tokType()==Token::eString) || (tok->varId() != 0)))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString(")")))
        return false;
...
// pattern: = %varid% + %num% ;
static bool match28(const Token* tok, const unsigned int varid) {
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("=")))
        return false;
    tok = tok->next();
    if (varid==0U)
...
// pattern: [;{}] %varid% =
static bool match29(const Token* tok, const unsigned int varid) {
    if (!tok || tok->str().size()!=1U || !strchr(";{}", tok->str()[0]))
        return false;
    tok = tok->next();
    if (varid==0U)
...
// pattern: strncpy|memcpy|memmove ( %varid% , %str% , %num% )
static bool match34(const Token* tok, const unsigned int varid) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("strncpy")) || (tok->str()==MatchCompiler::makeConstString("memcpy")) || (tok->str()==MatchCompiler::makeConstString("memmove"))))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("(")))
        return false;
...
// pattern: strncpy|strncat ( %varid% ,
static bool match35(const Token* tok, const unsigned int varid) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("strncpy")) || (tok->str()==MatchCompiler::makeConstString("strncat"))))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("(")))
        return false;
...
// pattern: ; strncat ( %varid% ,
static bool match38(const Token* tok, const unsigned int varid) {
    if (!tok || !(tok->str()==MatchCompiler::makeConstString(";")))
        return false;
    tok = tok->next();
...
// pattern: strcpy ( %varid% , %str% )
static bool match39(const Token* tok, const unsigned int varid) {
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("strcpy")))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("(")))
        return false;
...
static bool match49(const Token* tok) {
    ...
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("=")))
        return false;
    tok = tok->next();
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("malloc")) || (tok->str()==MatchCompiler::makeConstString("alloca"))))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("(")))
        return false;
...
// pattern: malloc|g_malloc|g_try_malloc|alloca ( strlen ( %var% ) )
static bool match59(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("malloc")) || (tok->str()==MatchCompiler::makeConstString("g_malloc")) || (tok->str()==MatchCompiler::makeConstString("g_try_malloc")) || (tok->str()==MatchCompiler::makeConstString("alloca"))))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("(")))
        return false;
...
// pattern: realloc|g_realloc|g_try_realloc ( %name% , strlen ( %var% ) )
static bool match61(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("realloc")) || (tok->str()==MatchCompiler::makeConstString("g_realloc")) || (tok->str()==MatchCompiler::makeConstString("g_try_realloc"))))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("(")))
        return false;
...
// pattern: main ( int %var% , char * %var% [ ] ,|)
static bool match64(const Token* tok) {
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("main")))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("(")))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("int")))
        return false;
    tok = tok->next();
    if (!tok || !(tok->varId() != 0))
        return false;
...
static bool bailoutIfSwitch(const Token *tok, const unsigned int varid)
{
    ...
    if (match2(end)) // scan the else-block
        end = end->linkAt(2);
    if (match3(end)) // Ticket #5203: Invalid code, bailout
        return true;

    // Used later to check if the body belongs to a "if"
    const bool is_if = tok->str() == MatchCompiler::makeConstString("if");

    for (; tok && tok != end; tok = tok->next()) {
        // If scanning a "if" block then bailout for "break"
        if (is_if && (tok->str() == MatchCompiler::makeConstString("break") || tok->str() == MatchCompiler::makeConstString("continue")))
            return true;
...
void CheckBufferOverrun::checkFunctionParameter(const Token &ftok, unsigned int paramIndex, const ArrayInfo &arrayInfo, const std::list<const Token *>& callstack)
{
    ...
    // only 1-dimensional arrays can be checked currently
        ...
        if (func && func->hasBody()) {
            ...
            for (const Token* ftok2 = func->functionScope->bodyStart; ftok2 != func->functionScope->bodyEnd; ftok2 = ftok2->next()) {
                ...
                if (ftok2->str() == MatchCompiler::makeConstString("}"))
                    break;

                if (ftok2->varId() == parameter->declarationId()) {
                    if (match9(ftok2->previous()) ||
                        match10(ftok2))
                        break;

                    if (match11(ftok2->previous())) {
    ...
    if (mSettings->isEnabled(Settings::WARNING)) {
        ...
        if (func) {
            ...
            if (argument && match13(argument->typeStartToken())
                ...
                if (match15(tok2) && arraysize > 0 && argsize > arraysize)
...
void CheckBufferOverrun::checkFunctionCall(const Token *tok, const ArrayInfo &arrayInfo, std::list<const Token *> callstack)
{
    ...
    while (argtok) {
        if (match16(argtok, declarationId))
            checkFunctionParameter(*tok, argnr, arrayInfo, callstack);
        else if (match17(argtok, declarationId)) {
...
void CheckBufferOverrun::checkScope(const Token *tok, const std::vector<const std::string*> &varname, const ArrayInfo &arrayInfo)
{
    ...
    if ((declarationId > 0 && match18(tok, declarationId)) ||
    ...
    for (const Token* const end = tok->scope()->bodyEnd; tok && tok != end; tok = tok->next()) {
        if (declarationId != 0 && match19(tok, declarationId)) {
            // Abort
            break;
        }

        // reassign buffer
        if (declarationId > 0 && match20(tok, declarationId)) {
            // using varid .. bailout
        ...
        // Array index..
        if ((declarationId > 0 && ((tok->str() == MatchCompiler::makeConstString("return") || (!tok->isName() && !match21(tok))) && match18(tok->next(), declarationId))) ||
            (declarationId == 0 && ((tok->str() == MatchCompiler::makeConstString("return") || (!tok->isName() && !match21(tok))) && (Token::Match(tok->next(), (varnames + " [").c_str()) || Token::Match(tok->next(), (*varname[0] +" [ %num% ] . " + *varname[1] + " [ %num% ]").c_str()))))) {
            std::vector<MathLib::bigint> indexes;
            const Token *tok2 = tok->tokAt(2 + varcount);
            for (; match22(tok2); tok2 = tok2->tokAt(3)) {
                const MathLib::bigint index = MathLib::toLongNumber(tok2->strAt(1));
                indexes.push_back(index);
            }
            for (; match22(tok2->tokAt(3)); tok2 = tok2->tokAt(3)) {
                const MathLib::bigint index = MathLib::toLongNumber(tok2->strAt(4));
                indexes.push_back(index);
            }
            if (indexes.empty() && arrayInfo.num().size() == 1U && match23(tok2) && tok2->astOperand2()) {
            ...
            if (indexes.size() == arrayInfo.num().size()) {
                ...
                while (tok3 && match24(tok3->previous()))
                    tok3 = tok3->tokAt(-2);

                // taking address of 1 past end?
                if (totalIndex == totalElements) {
                    const bool addr = (tok3 && (tok3->str() == MatchCompiler::makeConstString("&") ||
                                                match25(tok3->previous())));
                    if (addr)
                        continue;
                }
        ...
        if (declarationId == 0 && match26(tok)) {
        ...
        if (total_size > 0) {
            ...
            if ((declarationId > 0 && match27(tok, declarationId)) ||
                    ...
                    if (var && var->isArray() && var->dimensions().size() == 1) {
                        ...
                        if (len > total_size) {
                            if (printInconclusive)
                                possibleBufferOverrunError(tok, tok->strAt(4), tok->strAt(2), tok->str() == MatchCompiler::makeConstString("strcat"));
                            continue;
                        }
                    }
                }
        ...
        if (declarationId && match28(tok, declarationId)) {
            const MathLib::bigint index = MathLib::toLongNumber(tok->strAt(3));
            if (printPortability && index > size)
                pointerOutOfBoundsError(tok->tokAt(2));
            if (index >= size && match29(tok->tokAt(-2), declarationId))
                pointerIsOutOfBounds = true;
        }

        else if (pointerIsOutOfBounds && match30(tok, declarationId)) {
...
static std::vector<ValueFlow::Value> valueFlowGetArrayIndexes(const Token * const tok, bool conditional, std::size_t dimensions)
{
    ...
    for (const Token *tok2 = tok; indexes.size() < dimensions && match23(tok2); tok2 = tok2->link()->next()) {
        if (!tok2->astOperand2())
            return empty;
    ...
    /*
         {
            ...
            while (match31(parent))
                parent = parent->astParent();
            if (parent && !match32(parent))
                return;
        }
    */
    const bool printInconclusive = mSettings->inconclusive;
...
void CheckBufferOverrun::checkScope(const Token *tok, const ArrayInfo &arrayInfo)
{
    ...
    for (const Token* const end = tok->scope()->bodyEnd; tok != end; tok = tok->next()) {
        if (reassigned && tok->str() == MatchCompiler::makeConstString(";"))
            break;
...
void CheckBufferOverrun::checkScope_inner(const Token *tok, const ArrayInfo &arrayInfo)
{
    ...
    else if (printPortability && !tok->isCast() && astParent && astParent->str() == MatchCompiler::makeConstString("+")) {
        // undefined behaviour: result of pointer arithmetic is out of bounds
        const Token *index;
    ...
    else if (printPortability && astParent && astParent->str() == MatchCompiler::makeConstString("-")) {
    ...
    if (tok2) {
        while (tok2->astParent() && !match33(tok2->astParent()))
            tok2 = tok2->astParent();
        while (tok2->astParent() && tok2->astParent()->str() == MatchCompiler::makeConstString(","))
            tok2 = tok2->astParent();
        if (tok2->astParent() && tok2->astParent()->str() == MatchCompiler::makeConstString("("))
            tok2 = tok2->astParent();
        ...
        if (printWarning && printInconclusive && match34(tok2, arrayInfo.declarationId())) {
        ...
        if (printWarning && match35(tok2, arrayInfo.declarationId()) && match36(tok2->linkAt(1)->tokAt(-2))) {
            const Token* param3 = tok2->linkAt(1)->previous();

            // check for strncpy which is not terminated
            if (tok2->str() == MatchCompiler::makeConstString("strncpy")) {
                ...
                if (printInconclusive && num >= total_size) {
                    ...
                    for (; tok4; tok4 = tok4->next()) {
                        ...
                        if (tok4->varId() == tok3->varId()) {
                            ...
                            if (match18(tok4, tok3->varId()) && match37(tok4->linkAt(1)))
            ...
            else if (tok2->str() == MatchCompiler::makeConstString("strncat")) {
            ...
            if (match38(param3->tokAt(2), arrayInfo.declarationId()) && match36(param3->linkAt(4)->tokAt(-2))) {
        ...
        // Writing data into array..
        if (total_size > 0) {
            if (match39(tok2, arrayInfo.declarationId())) {
            ...
            while (match40(tok3, arrayInfo.declarationId())) {
...
void CheckBufferOverrun::negativeArraySize()
{
    ...
    for (const Variable *var : symbolDatabase->variableList()) {
        ...
        if (!match41(nameToken) || !nameToken->next()->astOperand2())
            continue;
...
void CheckBufferOverrun::checkGlobalAndLocalVariable()
{
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        if (match44(tok) && tok->next()->astOperand2()) {
        ...
        if (match41(tok) && tok->next()->astOperand2() && tok->variable() && tok->variable()->isPointer()) {
            const ValueFlow::Value *value = tok->next()->astOperand2()->getMaxValue(false);
            if (!value)
                continue;
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        ...
        for (const Token *tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (mTokenizer->isCPP() && match47(tok)) {
                tok = tok->tokAt(5);
            ...
            } else if (mTokenizer->isCPP() && match48(tok)) {
                size = 1;
                tok = tok->tokAt(5);
                if (tok->str() == MatchCompiler::makeConstString(";"))
                    nextTok = tok->next();
                    ...
                    nextTok = tok->link()->next();
            } else if (match49(tok) && match50(tok->linkAt(4))) {
                tok = tok->tokAt(4);
                if (tok->astOperand2() == nullptr || tok->astOperand2()->getMaxValue(false) == nullptr)
                    continue;
                size = tok->astOperand2()->getMaxValue(false)->intvalue;
                ...
                for (const Token *tok3 = func_scope->classDef; tok3 && tok3 != func_scope->bodyEnd; tok3 = tok3->next()) {
                    ...
                    if (scope->type == Scope::eStruct && var->isPublic()) {
                        ...
                        if (var->dimensions().size() == 1 && var->dimension(0) < 2 &&
                            ...
                            if (tok3->next()->str() == MatchCompiler::makeConstString("*")) {
                                // check for allocation
                                if ((match53(tok3->tokAt(3)) ||
                                     (match54(tok3->tokAt(3)) &&
                                      match55(tok3->linkAt(6)))) &&
                                    (tok3->strAt(4) == tok3->strAt(2))) {
                                    MathLib::bigint size;

                                    // find size of allocation
                                    if (tok3->strAt(3) == MatchCompiler::makeConstString("(")) // has cast
...
void CheckBufferOverrun::bufferOverrun()
{
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        if (mSettings->isEnabled(Settings::PORTABILITY) && tok->str() == MatchCompiler::makeConstString("+") && tok->valueType() && tok->valueType()->pointer > 0) {
            ...
            while (pointerToken && pointerToken->str() == MatchCompiler::makeConstString("."))
        ...
        if (tok->astParent() && tok->astParent()->str() == MatchCompiler::makeConstString(".")) {
            const Token *parent = tok->astParent();
            while (parent->astParent() && parent->astParent()->str() == MatchCompiler::makeConstString("."))
                parent = parent->astParent();
        ...
        if (strtoken && !var->isArray()) {
            ...
            if (var->dimension(0) <= 1 && match58(var->nameToken()->linkAt(1)))
                continue;
...
void CheckBufferOverrun::checkBufferAllocatedWithStrlen()
{
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        for (const Token *tok = scope->bodyStart->next(); tok && tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (!dstVarId || tok->strAt(1) != MatchCompiler::makeConstString("="))
                continue;
            ...
            if (match59(tok)) {
                ...
                tok      = varTok->tokAt(2);
            } else if (mTokenizer->isCPP() && match60(tok)) {
...
void CheckBufferOverrun::checkStringArgument()
{
    ...
    for (const Scope * const scope : symbolDatabase->functionScopes) {
        for (const Token *tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            if (!match12(tok) || !mSettings->library.hasminsize(tok->str()))
                continue;
        ...
        if (function) {
            ...
            for (const Token* end = tok->link(); tok != end; tok = tok->next()) {
                ...
                if (match66(tok)) {
                    ...
                    if (match67(tok, varid) || match18(tok, varid))
    ...
    : mVarName(var->name()), mDeclarationId((forcedeclid == 0U) ? var->declarationId() : forcedeclid)
{
    ...
    if (var->typeEndToken()->str() == MatchCompiler::makeConstString("*"))
        mElementSize = symbolDatabase->sizeOfType(var->typeEndToken());
    else if (var->typeStartToken()->strAt(-1) == MatchCompiler::makeConstString("struct"))
        mElementSize = 100;
    else {
        mElementSize = symbolDatabase->sizeOfType(var->typeEndToken());
    }
    ...
    for (const Scope * const scope : symbolDatabase->functionScopes) {
        for (const Token *tok = scope->bodyStart; tok && tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (match69(tok)) {
                ...
                if (match70(tok2, indexID))
                    arrayIndexThenCheckError(tok, indexName);
                else if (match71(tok2, indexID))
                    arrayIndexThenCheckError(tok, indexName);
            }
        }
    }
