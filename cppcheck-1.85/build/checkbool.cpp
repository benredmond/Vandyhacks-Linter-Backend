...
// pattern: (|.|return|&&|%oror%|throw|, %var% [&|]
static bool match3(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("(")) || (tok->str()==MatchCompiler::makeConstString(".")) || (tok->str()==MatchCompiler::makeConstString("return")) || (tok->str()==MatchCompiler::makeConstString("&&")) || (tok->tokType() == Token::eLogicalOp && tok->str()==MatchCompiler::makeConstString("||")) || (tok->str()==MatchCompiler::makeConstString("throw")) || (tok->str()==MatchCompiler::makeConstString(","))))
        return false;
    tok = tok->next();
    if (!tok || !(tok->varId() != 0))
        return false;
...
// pattern: ;
template<class T> static T * findmatch9(T * start_tok) {
...
{
    return (var && var->typeEndToken()->isStandardType() && !match1(var->typeEndToken()));
}
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            if (match3(tok)) {
                ...
                if (isBool(var)) {
                    bitwiseOnBooleanError(tok->next(), var->name(), tok->strAt(2) == MatchCompiler::makeConstString("&") ? "&&" : "||");
                    tok = tok->tokAt(2);
                }
            } else if (match4(tok) && (!tok->previous() || !tok->previous()->isExtendedOp() || tok->strAt(-1) == MatchCompiler::makeConstString(")") || tok->strAt(-1) == MatchCompiler::makeConstString("]"))) {
                const Variable *var = tok->next()->variable();
                if (isBool(var)) {
                    bitwiseOnBooleanError(tok->next(), var->name(), tok->str() == MatchCompiler::makeConstString("&") ? "&&" : "||");
                    tok = tok->tokAt(2);
                }
            }
        }
...
void CheckBool::checkComparisonOfFuncReturningBool()
{
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            if (tok->tokType() != Token::eComparisonOp || tok->str() == MatchCompiler::makeConstString("==") || tok->str() == MatchCompiler::makeConstString("!="))
                continue;
            ...
            while (secondToken->str() == MatchCompiler::makeConstString("!")) {
...
void CheckBool::checkAssignBoolToPointer()
{
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            if (tok->str() == MatchCompiler::makeConstString("=") && astIsBool(tok->astOperand2())) {
                const Token *lhs = tok->astOperand1();
                while (lhs && (lhs->str() == MatchCompiler::makeConstString(".") || lhs->str() == MatchCompiler::makeConstString("::")))
                    lhs = lhs->astOperand2();
...
void CheckBool::checkComparisonOfBoolExpressionWithInt()
{
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (boolExpr->isOp() && numTok->isName() && match6(tok))
...
void CheckBool::pointerArithBool()
{
    ...
    for (const Scope &scope : symbolDatabase->scopeList) {
        ...
        if (scope.type == Scope::eFor) {
            tok = findmatch9(scope.classDef->tokAt(2)) ;
            if (tok)
                tok = tok->astOperand2();
            if (tok)
                tok = tok->astOperand1();
