...
// pattern: class|struct|union|namespace ::| %name% {|:|::|<
...
// pattern: new|friend|const|enum|typedef|mutable|volatile|)|(|<
static bool match5(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("new")) || (tok->str()==MatchCompiler::makeConstString("friend")) || (tok->str()==MatchCompiler::makeConstString("const")) || (tok->str()==MatchCompiler::makeConstString("enum")) || (tok->str()==MatchCompiler::makeConstString("typedef")) || (tok->str()==MatchCompiler::makeConstString("mutable")) || (tok->str()==MatchCompiler::makeConstString("volatile")) || (tok->str()==MatchCompiler::makeConstString(")")) || (tok->str()==MatchCompiler::makeConstString("(")) || (tok->str()==MatchCompiler::makeConstString("<"))))
        return false;
    return true;
}
...
static bool match43(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("=")) || (tok->str()==MatchCompiler::makeConstString(",")) || (tok->str()==MatchCompiler::makeConstString("(")) || (tok->str()==MatchCompiler::makeConstString("return"))))
        return false;
    return true;
}
...
// pattern: {|;
template<class T> static T * findmatch55(T * start_tok) {
...
static bool match59(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("const")) || (tok->str()==MatchCompiler::makeConstString("noexcept")) || (tok->str()==MatchCompiler::makeConstString("throw")) || (tok->str()==MatchCompiler::makeConstString("override")) || (tok->str()==MatchCompiler::makeConstString("final")) || (tok->str()==MatchCompiler::makeConstString("volatile"))))
        return false;
    return true;
}
// pattern: ;|{|=
...
// pattern: static|const|volatile %any%
static bool match79(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("static")) || (tok->str()==MatchCompiler::makeConstString("const")) || (tok->str()==MatchCompiler::makeConstString("volatile"))))
        return false;
    tok = tok->next();
    if (!tok || false)
        return false;
...
// pattern: <
template<class T> static T * findmatch104(T * start_tok) {
    for (; start_tok; start_tok = start_tok->next()) {
...
// pattern: ,|>
template<class T> static T * findmatch105(T * start_tok) {
    for (; start_tok; start_tok = start_tok->next()) {
...
// pattern: ;|{
template<class T> static T * findmatch123(T * start_tok) {
    for (; start_tok; start_tok = start_tok->next()) {
...
// pattern: const|volatile|static|mutable|extern
static bool match124(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("const")) || (tok->str()==MatchCompiler::makeConstString("volatile")) || (tok->str()==MatchCompiler::makeConstString("static")) || (tok->str()==MatchCompiler::makeConstString("mutable")) || (tok->str()==MatchCompiler::makeConstString("extern"))))
        return false;
    return true;
}
...
// pattern: wchar_t|char|short|int|long
static bool match151(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("wchar_t")) || (tok->str()==MatchCompiler::makeConstString("char")) || (tok->str()==MatchCompiler::makeConstString("short")) || (tok->str()==MatchCompiler::makeConstString("int")) || (tok->str()==MatchCompiler::makeConstString("long"))))
        return false;
    return true;
}
// pattern: %str% ,|)
...
static bool match154(const Token* tok) {
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("wchar_t")))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("*")))
        return false;
...
static bool match162(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString(">")) || (tok->isName() && tok->varId()==0U && !tok->isKeyword())))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("::")))
        return false;
...
static bool match173(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString(";")) || (tok->str()==MatchCompiler::makeConstString("{")) || (tok->str()==MatchCompiler::makeConstString("}")) || (tok->str()==MatchCompiler::makeConstString("(")) || (tok->str()==MatchCompiler::makeConstString("const"))))
        return false;
    tok = tok->next();
...
static const Token* skipScopeIdentifiers(const Token* tok)
{
    ...
    while (match1(tok) ||
           (match2(tok) && match3(tok->linkAt(1)))) {
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok ? tok->next() : nullptr) {
        ...
        if (mErrorLogger)
            mErrorLogger->reportProgress(mTokenizer->list.getSourceFilePath(),
                                         ...
                                         tok->progressValue());
        // Locate next class
        if ((mTokenizer->isCPP() && ((match4(tok) &&
                                      !match5(tok->previous())) ||
                                     (match6(tok) ||
                                      match7(tok))))
            || (mTokenizer->isC() && match8(tok))) {
            ...
            // skip over final
            if (mTokenizer->isCPP() && match11(tok2))
                tok2 = tok2->next();
            ...
            if (!match12(tok2)) {
                ...
                if (tok2 && tok2->next()) {
                    if (tok2->next()->str() == MatchCompiler::makeConstString(";"))
                        tok = tok2->next();
                    else if (match13(tok2->next()) &&
                             match14(tok2->linkAt(2)))
                        tok = tok2->linkAt(2)->next();
                    else if (match15(tok2->next()) &&
                             tok2->next()->link()->strAt(1) == MatchCompiler::makeConstString(";"))
                        tok = tok2->next()->link()->next();
                    // skip variable declaration
                    else if (match16(tok2))
                        continue;
                    else if (match17(tok2) && mTokenizer->isFunctionHead(tok2->next(), "{;"))
                        continue;
                    else if (match18(tok2))
                        continue;
                    // skip template
                    else if (match19(tok2) &&
        ...
        // using namespace
        else if (mTokenizer->isCPP() && match27(tok)) {
        ...
        // using type alias
        else if (mTokenizer->isCPP() && match29(tok)) {
        ...
        // check if in class or structure or union
                ...
                if (tok->previous()->str() != MatchCompiler::makeConstString("::") || tok->strAt(-2) == scope->className) {
                    ...
                    // inline function
                        ...
                        while (end && end->str() != MatchCompiler::makeConstString("{") && end->str() != MatchCompiler::makeConstString(";")) {
                            if (end->link() && match36(end)) {
                                end = end->link();
                            } else if (foundInitList &&
                                       match37(end) &&
                                       match38(end->linkAt(1))) {
                                end = end->linkAt(1);
                            } else {
            ...
            else if (mTokenizer->isCPP() && match39(tok)) {
            ...
            if (match40(tok)) {
                ...
                scope = &scopeList.back();
            } else if (match41(tok) && match25(tok->next()->link())) {
                ...
                if (tok->previous()->varId())
                    ...
                    while (!match42(tok2))
                        tok2 = tok2->previous();
                    if (tok2->next() != tok && tok2->strAt(1) != MatchCompiler::makeConstString("."))
                        tok2 = nullptr; // No lambda

                    if (tok2 && tok2->str() == MatchCompiler::makeConstString(")") && tok2->link()->strAt(-1) == MatchCompiler::makeConstString("]")) {
                        scopeList.emplace_back(this, tok2->link()->linkAt(-1), scope, Scope::eLambda, tok);
                        scope->nestedList.push_back(&scopeList.back());
                        scope = &scopeList.back();
                    } else if (!match43(tok->previous()) && !(tok->strAt(-1) == MatchCompiler::makeConstString(")") && match43(tok->linkAt(-1)->previous()))) {
...
void SymbolDatabase::createSymbolDatabaseSetScopePointers()
{
    ...
    for (std::list<Scope>::iterator it = scopeList.begin(); it != scopeList.end(); ++it) {
        ...
        for (Token* tok = start; tok != end; tok = tok->next()) {
            if (start != end && tok->str() == MatchCompiler::makeConstString("{")) {
...
void SymbolDatabase::createSymbolDatabaseSetFunctionPointers(bool firstPass)
{
    ...
    for (const Token* tok = mTokenizer->list.front(); tok != mTokenizer->list.back(); tok = tok->next()) {
        if (!tok->function() && tok->varId() == 0 && match17(tok) && !isReservedName(tok->str())) {
...
void SymbolDatabase::createSymbolDatabaseSetVariablePointers()
{
    ...
    for (const Token* tok = mTokenizer->list.front(); tok != mTokenizer->list.back(); tok = tok->next()) {
        ...
        if (tok->variable() != nullptr &&
            ...
            if (match47(tok2))
                membertok = tok2->next();
            else if (match48(tok2) && tok->strAt(-1) == MatchCompiler::makeConstString("("))
        ...
        // func(...)[...].var
                 ...
                 (match50(tok->next()->link()) ||
                  (match51(tok->next()->link()) && match52(tok->next()->link()->next()->link())))) {
                ...
                if (dimension.num == 0) {
                    ...
                    if (dimension.start && (dimension.start == dimension.end)) {
                        ...
                        // check for a variable
                            ...
                            if (index_type->str() == MatchCompiler::makeConstString("char")) {
                            ...
                            } else if (index_type->str() == MatchCompiler::makeConstString("short")) {
                                if (index_type->isUnsigned())
                            ...
                            else if (index_type->str() == MatchCompiler::makeConstString("int")) {
                                if (index_type->isUnsigned())
                                    dimension.num = UINT_MAX + 1ULL;
                                else
                                    dimension.num = INT_MAX + 1ULL;
                            } else if (index_type->str() == MatchCompiler::makeConstString("long")) {
                                if (index_type->isUnsigned()) {
...
bool SymbolDatabase::isFunction(const Token *tok, const Scope* outerScope, const Token **funcStart, const Token **argStart, const Token** declEnd) const
{
    ...
    if (false && tok->str() == MatchCompiler::makeConstString("(") && tok->strAt(1) != MatchCompiler::makeConstString("*") &&
        (tok->link()->previous()->str() == MatchCompiler::makeConstString(")") || match53(tok->link()->tokAt(-2)))) {
        const Token* tok2 = tok->link()->next();
        if (tok2 && tok2->str() == MatchCompiler::makeConstString("(") && match54(tok2->link()->next())) {
            ...
            *declEnd = findmatch55(tok2->link()->next()) ;
            return true;
    ...
    // regular function?
    else if (match17(tok) && !isReservedName(tok->str()) && tok->previous() &&
             (match56(tok->previous()) || // Either a return type or scope qualifier in front of tok
        ...
        while (match57(tok1)) {
            ...
            if (match58(tok1))
                tok1 = tok1->previous();
            else if (tok1 && tok1->str() == MatchCompiler::makeConstString(">") && tok1->link() && match58(tok1->link()->previous()))
        ...
        if (tok2 && tok2->str() == MatchCompiler::makeConstString(".")) {
            for (tok2 = tok2->next(); tok2; tok2 = tok2->next()) {
                if (match60(tok2))
                    break;
                if (tok2->link() && match61(tok2))
                    tok2 = tok2->link();
            }
        }

        // done if constructor or destructor
        if (!match62(tok1) && tok1) {
            // skip over pointers and references
            while (match63(tok1) && !endsWith(tok1->str(), ':') && (!isReservedName(tok1->str()) || tok1->str() == MatchCompiler::makeConstString("const")))
                tok1 = tok1->previous();
            ...
            if (!match65(tok1) && tok1)
                return false;
        }

        if (tok2 &&
            (match60(tok2) ||
             (tok2->isUpperCaseName() && match66(tok2)) ||
             (tok2->isUpperCaseName() && match17(tok2) && tok2->next()->link()->strAt(1) == MatchCompiler::makeConstString("{")) ||
             match67(tok2) ||
             match68(tok2) ||
             match69(tok2))) {
            *funcStart = tok;
    ...
    // UNKNOWN_MACRO(a,b) { ... }
             ...
             match17(tok) &&
             tok->isUpperCaseName() &&
             match25(tok->linkAt(1)) &&
             (!tok->previous() || match70(tok->previous()))) {
        *funcStart = tok;
        *argStart = tok->next();
    ...
    // template constructor?
    else if (match2(tok) && match71(tok->next()->link())) {
        ...
        if (match72(tok2) ||
            ...
            *funcStart = tok;
            *argStart = tok2->link();
            *declEnd = findmatch55(tok2->next()) ;
            return true;
        }
    }

    // regular C function with missing return or invalid C++ ?
    else if (match17(tok) && !isReservedName(tok->str()) &&
             match25(tok->linkAt(1)) &&
             (!tok->previous() || match75(tok->previous()))) {
        if (mTokenizer->isC()) {
...
const Token * Variable::declEndToken() const
{
    ...
    while (declEnd && !match76(declEnd)) {
        if (declEnd->link() && match77(declEnd))
...
void Variable::evaluate(const Settings* settings)
{
    ...
    while (tok != end) {
        if (tok->str() == MatchCompiler::makeConstString("static"))
            ...
            setFlag(fIsPointer, !isArray() || match78(tok->previous()));
    ...
    while (match79(mTypeStartToken))
        mTypeStartToken = mTypeStartToken->next();
    while (mTypeEndToken && mTypeEndToken->previous() && match80(mTypeEndToken))
        mTypeEndToken = mTypeEndToken->previous();

    if (mTypeStartToken) {
        std::string strtype = mTypeStartToken->str();
        for (const Token *typeToken = mTypeStartToken; match81(typeToken); typeToken = typeToken->tokAt(2))
        ...
        setFlag(fIsStlType, match82(mTypeStartToken));
        setFlag(fIsStlString, isStlType() && (match83(mTypeStartToken->tokAt(2)) || (match84(mTypeStartToken->tokAt(2)) && !match3(mTypeStartToken->linkAt(3)))));
    }
    if (mAccess == Argument) {
        tok = mNameToken;
        if (!tok) {
    ...
    if (mScope && mScope->isClassOrStruct()) {
        ...
        if ((match85(declEnd) && declEnd->strAt(1) == mNameToken->str()) ||
            match86(declEnd))
            setFlag(fHasDefault, true);
    }

    if (mTypeStartToken) {
        if (match87(mTypeStartToken))
...
void Variable::setValueType(const ValueType &valueType)
{
    ...
    if ((mValueType->pointer > 0) && (!isArray() || match78(mNameToken->previous())))
      ...
      mFlags(0)
{
    ...
    // class constructor/destructor
        ...
        isExplicit(tokenDef->previous()->str() == MatchCompiler::makeConstString("explicit"));
    }

    const Token *tok1 = tok;

    // look for end of previous statement
    while (tok1->previous() && !match88(tok1->previous())) {
        tok1 = tok1->previous();
        ...
        // Function template
        else if (tok1->link() && tok1->str() == MatchCompiler::makeConstString(">") && match89(tok1->link()->previous()))
            break;
    }

    // find the return type
    if (!isConstructor() && !isDestructor()) {
        if (argDef->link()->strAt(1) == MatchCompiler::makeConstString(".")) // Trailing return type
...
bool Function::argsMatch(const Scope *scope, const Token *first, const Token *second, const std::string &path, unsigned int path_length)
{
    ...
    // skip "struct"
    if (first->str() == MatchCompiler::makeConstString("struct") || first->str() == MatchCompiler::makeConstString("enum"))
        first = first->next();
    if (second->str() == MatchCompiler::makeConstString("struct") || second->str() == MatchCompiler::makeConstString("enum"))
        second = second->next();
    ...
    while (first->str() == second->str() &&
        ...
        // definition missing variable name
                 ...
                 (first->next()->str() == MatchCompiler::makeConstString(")") && second->next()->str() != MatchCompiler::makeConstString(")"))) {
            second = second->next();
            // skip default value assignment
            if (second->next()->str() == MatchCompiler::makeConstString("=")) {
                do {
                    second = second->next();
                } while (!match95(second->next()));
            }
        } else if (first->next()->str() == MatchCompiler::makeConstString("[") && second->next()->str() != MatchCompiler::makeConstString("["))
            second = second->next();
        ...
        // function missing variable name
                 ...
                 (second->next()->str() == MatchCompiler::makeConstString(")") && first->next()->str() != MatchCompiler::makeConstString(")"))) {
            first = first->next();
            // skip default value assignment
            if (first->next()->str() == MatchCompiler::makeConstString("=")) {
                do {
                    first = first->next();
                } while (!match95(first->next()));
            }
        } else if (second->next()->str() == MatchCompiler::makeConstString("[") && first->next()->str() != MatchCompiler::makeConstString("["))
            first = first->next();

        // argument list has different number of arguments
        else if (second->str() == MatchCompiler::makeConstString(")"))
            break;

        // ckeck for type * x == type x[]
        else if (match96(first->next()) &&
                 match97(second->next())) {
            do {
                first = first->next();
            } while (!match95(first->next()));
        ...
        // const after *
        else if (first->next()->str() == MatchCompiler::makeConstString("*") && first->strAt(2) != MatchCompiler::makeConstString("const") &&
                 second->next()->str() == MatchCompiler::makeConstString("*") && second->strAt(2) == MatchCompiler::makeConstString("const")) {
            first = first->next();
            second = second->tokAt(2);
        }

        // variable names are different
        else if ((match98(first->next()) &&
                  match99(second->next())) &&
        ...
        // variable with class path
        else if (arg_path_length && match58(first->next()) && first->strAt(1) != MatchCompiler::makeConstString("const")) {
            ...
            else if (arg_path_length <= 2 && match58(first->next()) &&
                     (match10(second->next()) ||
                      (match2(second->next()) &&
                       match100(second->linkAt(1)))) &&
...
const Token * Function::constructorMemberInitialization() const
{
    if (!isConstructor() || !functionScope || !functionScope->bodyStart)
        return nullptr;
    if (match17(token) && match102(token->linkAt(1)))
...
void SymbolDatabase::addClassFunction(Scope **scope, const Token **tok, const Token *argStart)
{
    ...
    while (tok1 && tok1->previous() && tok1->previous()->str() == MatchCompiler::makeConstString("::") && tok1->tokAt(-2) &&
           (tok1->tokAt(-2)->isName() ||
            (tok1->strAt(-2) == MatchCompiler::makeConstString(">") && tok1->linkAt(-2) && match58(tok1->linkAt(-2)->previous())))) {
        count++;
                ...
                if (!func->hasBody()) {
                    if (Function::argsMatch(scope1, func->argDef, (*tok)->next(), path, path_length)) {
                        if (func->type == Function::eDestructor && destructor) {
                            ...
                            // normal function?
                            if ((*tok)->next()->link()) {
                                const bool hasConstKeyword = (*tok)->next()->link()->next()->str() == MatchCompiler::makeConstString("const");
...
void SymbolDatabase::addNewFunction(Scope **scope, const Token **tok)
{
    ...
    while (tok1 && tok1->str() != MatchCompiler::makeConstString("{") && tok1->str() != MatchCompiler::makeConstString(";")) {
        if (tok1->link() && match36(tok1)) {
            tok1 = tok1->link();
        } else if (foundInitList &&
                   match37(tok1) &&
                   match38(tok1->linkAt(1))) {
            tok1 = tok1->linkAt(1);
        } else {
            if (tok1->str() == MatchCompiler::makeConstString(":"))
                foundInitList = true;
...
bool Variable::arrayDimensions(const Library* lib)
{
    ...
    if (container && container->arrayLike_indexOp && container->size_templateArgNo > 0) {
        const Token* tok = findmatch104(mTypeStartToken) ;
        if (tok) {
            ...
            if (tok) {
                ...
                dimension_.end = findmatch105(tok) ;
                if (dimension_.end)
...
void SymbolDatabase::printOut(const char *title) const
{
    ...
    for (std::list<Scope>::const_iterator scope = scopeList.begin(); scope != scopeList.end(); ++scope) {
        ...
        for (func = scope->functionList.begin(); func != scope->functionList.end(); ++func) {
            ...
            if (func->retDef) {
                std::cout << "           ";
                for (const Token * tok = func->retDef; tok && tok != func->tokenDef && !match106(tok); tok = tok->next())
                    std::cout << " " << tokenType(tok);
                std::cout << std::endl;
            }
...
void Function::addArguments(const SymbolDatabase *symbolDatabase, const Scope *scope)
{
    ...
    if (!(start && start->link() != start->next() && !match107(start)))
        return;
    ...
    for (const Token* tok = start->next(); tok; tok = tok->next()) {
        ...
        if (match95(typeTok)) { // #8333
            symbolDatabase->mTokenizer->syntaxError(typeTok);
            return;
        }
        // skip over qualification
        while (match28(typeTok))
            typeTok = typeTok->tokAt(2);

        // check for argument with no name or missing varid
        if (!endTok) {
            if (tok->previous()->isName() && !match80(tok->tokAt(-1))) {
    ...
    enumClass(false)
{
    ...
    if (nameTok && ((type == Scope::eEnum && match112(nameTok)) || nameTok->str() != MatchCompiler::makeConstString("{"))) // anonymous and unnamed structs/unions don't have a name
...
void Scope::getVariableList(const Settings* settings)
{
    ...
    for (const Token *tok = start; tok && tok != bodyEnd; tok = tok->next()) {
        ...
        // Is it a nested class or structure?
            ...
            if (match114(tok->next()->link())) {
                tok = tok->next()->link()->tokAt(2);
                continue;
            } else if (match14(tok->next()->link())) {
                tok = tok->next();
                continue;
            }
        }
        ...
        // skip case/default
        if (match117(tok)) {
            while (tok->next() && !match118(tok->next()))
                tok = tok->next();
            continue;
        }

        // Search for start of statement..
        else if (tok->previous() && !match119(tok->previous()))
            continue;
        else if (tok->str() == MatchCompiler::makeConstString(";"))
            continue;
...
const Token *Scope::checkVariable(const Token *tok, AccessControl varaccess, const Settings* settings)
{
    // Is it a throw..?
    if (match120(tok) &&
        match23(tok->linkAt(2))) {
        return tok->linkAt(2);
    }

    if ((match121(tok) &&
         match23(tok->linkAt(4)))) {
        return tok->linkAt(4);
    }

    // friend?
    if (match122(tok) && tok->next()->varId() == 0) {
        const Token *next = findmatch123(tok->tokAt(2)) ;
...
{
    while (match128(tok)  || (match129(tok) && match77(tok->link()->next()))) {
        tok = tok->next();
        if (tok->strAt(-1) == MatchCompiler::makeConstString("(") && match28(tok))
            tok = tok->tokAt(2);
    }
...
bool Scope::isVariableDeclaration(const Token* const tok, const Token*& vartok, const Token*& typetok) const
{
    ...
    if (match136(localVarTok) || (localVarTok && localVarTok->varId() && localVarTok->strAt(1) == MatchCompiler::makeConstString(":"))) {
        vartok = localVarTok;
        typetok = localTypeTok;
    } else if (match137(localVarTok) && localVarTok->str() != MatchCompiler::makeConstString("operator")) {
        vartok = localVarTok;
        typetok = localTypeTok;
    } else if (localVarTok && localVarTok->varId() && match138(localVarTok) &&
               match139(localVarTok->next()->link())) {
        vartok = localVarTok;
        typetok = localTypeTok;
    } else if (type == eCatch &&
               match140(localVarTok)) {
        vartok = localVarTok;
        typetok = localTypeTok;
    }
...
const Token * Scope::addEnum(const Token * tok, bool isCpp)
{
    ...
    if (tok2->str() == MatchCompiler::makeConstString("{")) {
        ...
        while (match141(tok2) ||
               (match17(tok2) && match142(tok2->linkAt(1)))) {
...
const Type* SymbolDatabase::findVariableType(const Scope *start, const Token *typeTok) const
{
    ...
    // check for a qualified name and use it when given
        ...
        while (match28(tok1->tokAt(-2)) ||
               (match3(tok1->tokAt(-2)) && tok1->linkAt(-2) && match135(tok1->linkAt(-2)->tokAt(-1)))) {
            if (tok1->strAt(-1) == MatchCompiler::makeConstString("::"))
                tok1 = tok1->tokAt(-2);
            else
        ...
        if (scope) {
            ...
            while (scope && (match28(tok1) ||
                             (match131(tok1) && match3(tok1->linkAt(1))))) {
                if (tok1->strAt(1) == MatchCompiler::makeConstString("::"))
                    tok1 = tok1->tokAt(2);
                else
                    tok1 = tok1->linkAt(1)->tokAt(2);
...
bool Scope::hasInlineOrLambdaFunction() const
{
    for (std::list<Scope*>::const_iterator it = nestedList.begin(); it != nestedList.end(); ++it) {
        const Scope *s = *it;
        // Inline function
        if (s->type == Scope::eUnconditional && match25(s->bodyStart->previous()))
            return true;
        // Lambda function
    ...
    if (callarg) {
        ...
        if (ptrequals && constEquals &&
            ...
            if (ptrequals && constEquals && funcarg->typeStartToken()->str() == MatchCompiler::makeConstString("void"))
                fallback1++;
            else if (constEquals && funcarg->isStlStringType() && match144(callarg->typeStartToken()))
                fallback2++;
        } else if (ptrequals) {
            const bool takesInt = match145(funcarg->typeStartToken());
            const bool takesFloat = match87(funcarg->typeStartToken());
            const bool passesInt = match145(callarg->typeStartToken());
            const bool passesFloat = match87(callarg->typeStartToken());
...
static bool valueTypeMatch(const ValueType * valuetype, const Token * type)
{
    ...
    return ((((type->str() == MatchCompiler::makeConstString("bool") && valuetype->type == ValueType::BOOL) ||
              (type->str() == MatchCompiler::makeConstString("char") && valuetype->type == ValueType::CHAR) ||
              (type->str() == MatchCompiler::makeConstString("short") && valuetype->type == ValueType::SHORT) ||
              (type->str() == MatchCompiler::makeConstString("int") && valuetype->type == ValueType::INT) ||
              ((type->str() == MatchCompiler::makeConstString("long") && type->isLong()) && valuetype->type == ValueType::LONGLONG) ||
              (type->str() == MatchCompiler::makeConstString("long") && valuetype->type == ValueType::LONG) ||
              (type->str() == MatchCompiler::makeConstString("float") && valuetype->type == ValueType::FLOAT) ||
              ((type->str() == MatchCompiler::makeConstString("double") && type->isLong()) && valuetype->type == ValueType::LONGDOUBLE) ||
              (type->str() == MatchCompiler::makeConstString("double") && valuetype->type == ValueType::DOUBLE)) &&
...
const Function* Scope::findFunction(const Token *tok, bool requireConst) const
{
    ...
    for (std::size_t i = 0; i < matches.size();) {
        ...
        for (std::size_t j = 0; j < args; ++j) {
            ...
            else if (match147(arguments[j])) {
                const Variable * callarg = check->getVariableFromVarId(arguments[j]->next()->varId());
                if (callarg) {
                    const bool funcargref = (funcarg->typeEndToken()->str() == MatchCompiler::makeConstString("&"));
                    if (funcargref &&
                        (callarg->typeStartToken()->str() == funcarg->typeStartToken()->str() &&
                         callarg->typeStartToken()->isUnsigned() == funcarg->typeStartToken()->isUnsigned() &&
                         callarg->typeStartToken()->isLong() == funcarg->typeStartToken()->isLong())) {
            ...
            else if (match148(arguments[j])) {
                const Variable * callarg = check->getVariableFromVarId(arguments[j]->next()->varId());
                if (callarg) {
                    const bool funcargptr = (funcarg->typeEndToken()->str() == MatchCompiler::makeConstString("*"));
                    if (funcargptr &&
                        (callarg->typeStartToken()->str() == funcarg->typeStartToken()->str() &&
                         callarg->typeStartToken()->isUnsigned() == funcarg->typeStartToken()->isUnsigned() &&
                         callarg->typeStartToken()->isLong() == funcarg->typeStartToken()->isLong())) {
                        same++;
                    } else if (funcargptr && funcarg->typeStartToken()->str() == MatchCompiler::makeConstString("void")) {
                        fallback1++;
                    } else {
                        // can't match so remove this function from possible matches
                        matches.erase(matches.begin() + i);
            ...
            // check for a match with a numeric literal
                ...
                if (MathLib::isInt(arguments[j]->str()) && (!funcarg->isPointer() || MathLib::isNullValue(arguments[j]->str()))) {
                    ...
                    if (arguments[j]->str().find("ll") != std::string::npos ||
                        ...
                        if (arguments[j]->str().find('u') != std::string::npos ||
                            ...
                            if (funcarg->typeStartToken()->isLong() &&
                                ...
                                funcarg->typeStartToken()->str() == MatchCompiler::makeConstString("long")) {
                                exactMatch = true;
                            }
                        } else {
                            if (funcarg->typeStartToken()->isLong() &&
                        ...
                        if (funcarg->typeStartToken()->isUnsigned() &&
                            funcarg->typeStartToken()->str() == MatchCompiler::makeConstString("int")) {
                            exactMatch = true;
                        } else if (match150(funcarg->typeStartToken())) {
                            exactMatch = true;
                        }
                    } else {
                        if (match151(funcarg->typeStartToken())) {
                            exactMatch = true;
                        }
                    }
                    ...
                    if (exactMatch)
                        ...
                        if (funcarg->isPointer() || match151(funcarg->typeStartToken()))
                            fallback1++;
                        else if (match87(funcarg->typeStartToken()))
                            fallback2++;
                    }
                    ...
                    if (arguments[j]->str().find('f') != std::string::npos ||
                        ...
                        if (funcarg->typeStartToken()->str() == MatchCompiler::makeConstString("float")) {
                            exactMatch = true;
                        }
                    } else if (arguments[j]->str().find('l') != std::string::npos ||
                               arguments[j]->str().find('L') != std::string::npos) {
                        if (funcarg->typeStartToken()->isLong() &&
                            funcarg->typeStartToken()->str() == MatchCompiler::makeConstString("double"))  {
                            exactMatch = true;
                        }
                    } else {
                        if (!funcarg->typeStartToken()->isLong() &&
                    ...
                    if (exactMatch)
                        ...
                        if (match87(funcarg->typeStartToken()))
                            fallback1++;
                        else if (match151(funcarg->typeStartToken()))
                            fallback2++;
                    }
                }
            }
            ...
            else if (match152(arguments[j])) {
                if (funcarg->typeStartToken() != funcarg->typeEndToken() &&
                    ((!arguments[j]->isLong() && match153(funcarg->typeStartToken())) ||
                     (arguments[j]->isLong() && match154(funcarg->typeStartToken()))))
                    same++;
                else if (match155(funcarg->typeStartToken()))
                    fallback1++;
            ...
            else if (!funcarg->isArrayOrPointer() && match156(arguments[j])) {
                if (arguments[j]->isLong() && funcarg->typeStartToken()->str() == MatchCompiler::makeConstString("wchar_t"))
                    same++;
                else if (!arguments[j]->isLong() && funcarg->typeStartToken()->str() == MatchCompiler::makeConstString("char"))
                    same++;
                else if (match151(funcarg->typeStartToken()))
                    fallback1++;
            }
            ...
            else if (!funcarg->isArrayOrPointer() && match157(arguments[j])) {
                if (match158(funcarg->typeStartToken()))
                    same++;
                else if (match151(funcarg->typeStartToken()))
                    fallback1++;
            }
            ...
            else if (funcarg->isPointer() && match159(arguments[j])) {
                same++;
            }

            // check that function argument type is not mismatching
            else if (funcarg->isReference() && arguments[j]->str() == MatchCompiler::makeConstString("&")) {
                // can't match so remove this function from possible matches
                matches.erase(matches.begin() + i);
                erased = true;
                break;
            ...
            // Try to evaluate the apparently more complex expression
                ...
                while (argtok->astParent() && argtok->astParent() != tok->next() && argtok->astParent()->str() != MatchCompiler::makeConstString(",")) {
                ...
                if (argtok && argtok->valueType()) {
                    ...
                    const bool constEquals = !isArrayOrPointer ||
                                             ((valuetype->constness > 0) == (funcarg->typeStartToken()->strAt(-1) == MatchCompiler::makeConstString("const")));
                    if (ptrequals && constEquals && valueTypeMatch(valuetype, funcarg->typeStartToken())) {
                        ...
                        const bool takesInt = match160(funcarg->typeStartToken()) ||
...
const Function* SymbolDatabase::findFunction(const Token *tok) const
{
    ...
    if (tok->strAt(-1) == MatchCompiler::makeConstString("::")) {
        ...
        if (currScope) {
            while (currScope && !(match163(tok1) ||
                                  (match131(tok1) && match164(tok1->linkAt(1))))) {
                if (tok1->strAt(1) == MatchCompiler::makeConstString("::"))
                    tok1 = tok1->tokAt(2);
                else
                    tok1 = tok1->linkAt(1)->tokAt(2);
...
const Scope *SymbolDatabase::findScope(const Token *tok, const Scope *startScope) const
{
    ...
    while (scope && tok && tok->isName()) {
        if (tok->strAt(1) == MatchCompiler::makeConstString("::")) {
            ...
            tok = tok->tokAt(2);
        } else if (tok->strAt(1) == MatchCompiler::makeConstString("<") && match3(tok->linkAt(1))) {
            scope = scope->findRecordInNestedList(tok->str());
            tok = tok->linkAt(1)->tokAt(2);
        } else
...
const Type* SymbolDatabase::findType(const Token *startTok, const Scope *startScope) const
{
    ...
    while (scope && tok && tok->isName()) {
        if (tok->strAt(1) == MatchCompiler::makeConstString("::") || (tok->strAt(1) == MatchCompiler::makeConstString("<") && match3(tok->linkAt(1)))) {
            scope = scope->findRecordInNestedList(tok->str());
            if (scope) {
                if (tok->strAt(1) == MatchCompiler::makeConstString("::"))
                    tok = tok->tokAt(2);
...
Function * SymbolDatabase::findFunctionInScope(const Token *func, const Scope *ns, const std::string & path, unsigned int path_length)
{
    ...
    if (!function) {
        ...
        if (scope && match169(func->tokAt(1))) {
...
void SymbolDatabase::setValueType(Token *tok, const ValueType &valuetype)
{
    ...
    if (parent->str() == MatchCompiler::makeConstString("[") && (!mIsCpp || parent->astOperand1() == tok) && valuetype.pointer > 0U && !match175(parent->previous())) {
    ...
    if (match17(parent->previous()) && parent->astOperand1() == tok && valuetype.pointer > 0U) {
        ValueType vt(valuetype);
        vt.pointer -= 1U;
        setValueType(parent, vt);
        return;
    ...
    if (vt2 &&
        ...
        match176(parent->astParent()) &&
        !parent->previous()->valueType() &&
        match177(parent->astParent()->astOperand1())) {
        const bool isconst = match178(parent->astParent()->next());
...
static const Token * parsedecl(const Token *type, ValueType * const valuetype, ValueType::Sign defaultSignedness, const Settings* settings)
{
    ...
    while (match179(type) && !type->variable() && !type->function()) {
        if (type->isSigned())
            valuetype->sign = ValueType::Sign::SIGNED;
        else if (type->isUnsigned())
            valuetype->sign = ValueType::Sign::UNSIGNED;
        if (valuetype->type == ValueType::Type::UNKNOWN_TYPE &&
        ...
        else if (type->str() == MatchCompiler::makeConstString("const"))
            ...
            while (match179(type) && !type->variable())
                type = type->next();
            break;
        ...
        else if (match181(type->previous()))
            valuetype->fromLibraryType(type->str(), settings);
...
void SymbolDatabase::setValueTypeInTokenList()
{
    ...
    for (Token *tok = tokens; tok; tok = tok->next()) {
        if (tok->isNumber()) {
            ...
            // cast
            if (!tok->astOperand2() && match182(tok)) {
                ValueType valuetype;
                if (match183(parsedecl(tok->next(), &valuetype, mDefaultSignedness, mSettings)))
                    setValueType(tok, valuetype);
            }

            // C++ cast
            if (tok->astOperand2() && match184(tok->astOperand1()) && tok->astOperand1()->linkAt(1)) {
                ValueType valuetype;
                if (match185(parsedecl(tok->astOperand1()->tokAt(2), &valuetype, mDefaultSignedness, mSettings)))
                    setValueType(tok, valuetype);
            }
        ...
        } else if (mIsCpp && tok->str() == MatchCompiler::makeConstString("new")) {
