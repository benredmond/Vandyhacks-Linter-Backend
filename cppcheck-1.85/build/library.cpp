    ...
    for (const tinyxml2::XMLElement *node = rootnode->FirstChildElement(); node; node = node->NextSiblingElement()) {
        ...
        if (nodename == MatchCompiler::makeConstString("memory") || nodename == MatchCompiler::makeConstString("resource")) {
            ...
            for (const tinyxml2::XMLElement *memorynode = node->FirstChildElement(); memorynode; memorynode = memorynode->NextSiblingElement()) {
                ...
                if (memorynodename == MatchCompiler::makeConstString("alloc")) {
                    AllocFunc temp;
            ...
            if (opLessAllowed)
                container.opLessAllowed = std::string(opLessAllowed) == MatchCompiler::makeConstString("true");
            ...
            for (const tinyxml2::XMLElement *containerNode = node->FirstChildElement(); containerNode; containerNode = containerNode->NextSiblingElement()) {
                ...
                if (containerNodeName == MatchCompiler::makeConstString("size") || containerNodeName == MatchCompiler::makeConstString("access") || containerNodeName == MatchCompiler::makeConstString("other")) {
                    for (const tinyxml2::XMLElement *functionNode = containerNode->FirstChildElement(); functionNode; functionNode = functionNode->NextSiblingElement()) {
                        if (std::string(functionNode->Name()) != MatchCompiler::makeConstString("function")) {
                        ...
                        if (action_ptr) {
                            ...
                            else if (actionName == MatchCompiler::makeConstString("clear"))
                                action = Container::CLEAR;
                            else if (actionName == MatchCompiler::makeConstString("push"))
                                action = Container::PUSH;
                            else if (actionName == MatchCompiler::makeConstString("pop"))
                                action = Container::POP;
                            else if (actionName == MatchCompiler::makeConstString("find"))
                                action = Container::FIND;
                            else if (actionName == MatchCompiler::makeConstString("insert"))
                                action = Container::INSERT;
                            else if (actionName == MatchCompiler::makeConstString("erase"))
                                action = Container::ERASE;
                            else if (actionName == MatchCompiler::makeConstString("change-content"))
                                action = Container::CHANGE_CONTENT;
                            else if (actionName == MatchCompiler::makeConstString("change-internal"))
                                action = Container::CHANGE_INTERNAL;
                            else if (actionName == MatchCompiler::makeConstString("change"))
                        ...
                        if (yield_ptr) {
                            ...
                            else if (yieldName == MatchCompiler::makeConstString("item"))
                                yield = Container::ITEM;
                            else if (yieldName == MatchCompiler::makeConstString("buffer"))
                                yield = Container::BUFFER;
                            else if (yieldName == MatchCompiler::makeConstString("buffer-nt"))
                                yield = Container::BUFFER_NT;
                            else if (yieldName == MatchCompiler::makeConstString("start-iterator"))
                                yield = Container::START_ITERATOR;
                            else if (yieldName == MatchCompiler::makeConstString("end-iterator"))
                                yield = Container::END_ITERATOR;
                            else if (yieldName == MatchCompiler::makeConstString("iterator"))
                                yield = Container::ITERATOR;
                            else if (yieldName == MatchCompiler::makeConstString("size"))
                                yield = Container::SIZE;
                            else if (yieldName == MatchCompiler::makeConstString("empty"))
                    ...
                    if (containerNodeName == MatchCompiler::makeConstString("size")) {
                        ...
                        if (indexArg)
                            container.arrayLike_indexOp = std::string(indexArg) == MatchCompiler::makeConstString("array-like");
                    }
                    ...
                    if (string)
                        container.stdStringLike = std::string(string) == MatchCompiler::makeConstString("std-like");
                } else
...
bool Library::isIntArgValid(const Token *ftok, int argnr, const MathLib::bigint argvalue) const
{
    ...
    for (const Token *tok = tokenList.front(); tok; tok = tok->next()) {
        if (tok->isNumber() && argvalue == MathLib::toLongNumber(tok->str()))
            return true;
        if (match2(tok) && argvalue >= MathLib::toLongNumber(tok->str()) && argvalue <= MathLib::toLongNumber(tok->strAt(2)))
            return true;
        if (match3(tok) && argvalue >= MathLib::toLongNumber(tok->str()))
            return true;
        if ((!tok->previous() || tok->previous()->str() == MatchCompiler::makeConstString(",")) && match4(tok) && argvalue <= MathLib::toLongNumber(tok->strAt(1)))
            return true;
    }
    return false;
}
...
bool Library::isFloatArgValid(const Token *ftok, int argnr, double argvalue) const
{
    ...
    gettokenlistfromvalid(ac->valid, tokenList);
    for (const Token *tok = tokenList.front(); tok; tok = tok->next()) {
        if (match2(tok) && argvalue >= MathLib::toDoubleNumber(tok->str()) && argvalue <= MathLib::toDoubleNumber(tok->strAt(2)))
            return true;
        if (match3(tok) && argvalue >= MathLib::toDoubleNumber(tok->str()))
            return true;
        if ((!tok->previous() || tok->previous()->str() == MatchCompiler::makeConstString(",")) && match4(tok) && argvalue <= MathLib::toDoubleNumber(tok->strAt(1)))
            return true;
    }
    return false;
}
...
{
    if (!match5(ftok) && (ftok->strAt(-1) != MatchCompiler::makeConstString("&") || ftok->previous()->astOperand2()))
        return "";
    ...
    if (match6(ftok->previous()))
        return "";
    if (!match7(ftok->tokAt(-2)))
        return ftok->str();
...
bool Library::isScopeNoReturn(const Token *end, std::string *unknownFunc) const
{
    ...
    if (match14(start) && match15(funcname)) {
    ...
    if (cond->str() == MatchCompiler::makeConstString("(")) {
        ...
        if (tok && tok->str() == MatchCompiler::makeConstString(".")) {
            if (tok->astOperand1() && tok->astOperand1()->valueType()) {
                ...
                return match16(cond) && cond->previous()->str() == fallback;
            }
        }
    }
    return false;
