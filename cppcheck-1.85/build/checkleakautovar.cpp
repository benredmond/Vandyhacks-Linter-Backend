...
// pattern: auto_ptr|unique_ptr|shared_ptr < %type%
static bool match21(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("auto_ptr")) || (tok->str()==MatchCompiler::makeConstString("unique_ptr")) || (tok->str()==MatchCompiler::makeConstString("shared_ptr"))))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("<")))
        return false;
...
// pattern: [ ]
template<class T> static T * findmatch23(T * start_tok, const Token * end) {
...
// pattern: & %name%
template<class T> static T * findmatch26(T * start_tok, const Token * end) {
    for (; start_tok && start_tok != end; start_tok = start_tok->next()) {
...
// pattern: %type%
template<class T> static T * findmatch28(T * start_tok, const Token * end) {
    for (; start_tok && start_tok != end; start_tok = start_tok->next()) {
...
static bool match37(const Token* tok, const unsigned int varid) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("return")) || (tok->str()==MatchCompiler::makeConstString("(")) || (tok->str()==MatchCompiler::makeConstString(","))))
        return false;
    tok = tok->next();
    if (varid==0U)
    ...
    for (const Token *tok = startToken; tok && tok != endToken; tok = tok->next()) {
        ...
        if (match2(varTok)) {
            ...
            if (mTokenizer->isCPP()) {
                ...
                if (!var->typeStartToken()->isStandardType() && match7(varTok))
                    continue;
            ...
            // allocation?
            if (tokRightAstOperand && match8(tokRightAstOperand->previous())) {
            ...
            } else if (mTokenizer->isCPP() && match9(varTok->tokAt(2))) {
            ...
            if (match10(varTok->tokAt(2)) && MathLib::toLongNumber(varTok->strAt(2)) != 0)
                notzero.insert(varTok->varId());
            else if (match11(varTok->tokAt(2)) && varTok->tokAt(3)->isUpperCaseName())
                notzero.insert(varTok->varId());
            else
        ...
        // if/else
            ...
            for (const Token *innerTok = tok->tokAt(2); innerTok && innerTok != closingParenthesis; innerTok = innerTok->next()) {
                ...
                if (match2(innerTok)) {
                    ...
                    } else if (mTokenizer->isCPP() && match9(innerTok->tokAt(2))) {
            ...
            if (match13(closingParenthesis)) {
                ...
                while (!tokens.empty()) {
                    ...
                    if (tok3->str() == MatchCompiler::makeConstString("(") && match14(tok3->astOperand1())) {
                        tokens.push(tok3->astOperand2());
                        continue;
                    } else if (tok3->str() == MatchCompiler::makeConstString("(") && match15(tok3->previous())) {
        ...
        else if ((match8(tok) && match13(tok->linkAt(1))) || match17(tok)) {
            varInfo->clear();
            break;
        }
        ...
        // throw
        else if (mTokenizer->isCPP() && tok->str() == MatchCompiler::makeConstString("throw")) {
            bool tryFound = false;
        ...
        // Function call..
            ...
            if (allocation.status == VarInfo::NOALLOC && match18(tok)) {
        ...
        // delete
        else if (mTokenizer->isCPP() && tok->str() == MatchCompiler::makeConstString("delete")) {
            const bool arrayDelete = (tok->strAt(1) == MatchCompiler::makeConstString("["));
            if (arrayDelete)
                tok = tok->tokAt(3);
            else
                tok = tok->next();
        ...
        // Check smart pointer
        else if (match21(ftok)) {


            const Token * typeEndTok = ftok->linkAt(1);
            if (!match22(typeEndTok))
                continue;
            ...
            if (findmatch23(ftok->next(), typeEndTok) )
                arrayDelete = true;
            ...
            if (deleterToken) {
                ...
                const Token * dtok = findmatch26(deleterToken, endDeleterToken) ;
                if (dtok) {
                    ...
                    if (deleterToken->str() == MatchCompiler::makeConstString("[") &&
                        match27(deleterToken->link()) &&
                        // TODO: Check for mutable keyword
                        match13(deleterToken->link()->linkAt(1))) {
                    ...
                    } else if ((dtok = findmatch28(deleterToken, endDeleterToken) ) && dtok->type()) {
...
const Token * CheckLeakAutoVar::checkTokenInsideExpression(const Token * const tok, VarInfo *varInfo)
{
    ...
    if (tok->varId() > 0) {
        ...
        if (var != varInfo->alloctype.end()) {
            ...
            } else if (match29(tok->tokAt(-2))) {
                varInfo->erase(tok->varId());
            } else if (tok->strAt(-1) == MatchCompiler::makeConstString("=")) {
                varInfo->erase(tok->varId());
            }
        } else if (match30(tok->previous())) {
...
void CheckLeakAutoVar::changeAllocStatus(VarInfo *varInfo, const VarInfo::AllocInfo& allocation, const Token* tok, const Token* arg)
{
    ...
    if (var != alloctype.end()) {
        if (allocation.status == VarInfo::NOALLOC) {
            ...
            if (var->second.status == VarInfo::DEALLOC && arg->previous()->str() == MatchCompiler::makeConstString("&"))
...
void CheckLeakAutoVar::functionCall(const Token *tokName, const Token *tokOpeningPar, VarInfo *varInfo, const VarInfo::AllocInfo& allocation, const Library::AllocFunc* af)
{
    ...
    for (const Token *arg = tokFirstArg; arg; arg = arg->nextArgument()) {
        if (mTokenizer->isCPP() && arg->str() == MatchCompiler::makeConstString("new")) {
            arg = arg->next();
            if (match31(arg))
                arg = arg->tokAt(5);
        }
        ...
        // Check smart pointer
            ...
            if (!match35(typeEndTok))
                continue;

            bool arrayDelete = false;
            if (findmatch23(arg->next(), typeEndTok) )
                arrayDelete = true;
            ...
            if (deleterToken) {
                ...
                if (dtok) {
                    ...
                    // If the deleter is a class, check if class calls the dealloc function
                    dtok = findmatch28(deleterToken, endDeleterToken) ;
                    if (dtok && dtok->type()) {
                        const Scope * tscope = dtok->type()->classScope;
    ...
    for (std::map<unsigned int, VarInfo::AllocInfo>::const_iterator it = alloctype.begin(); it != alloctype.end(); ++it) {
        ...
        if (var) {
            ...
            for (const Token *tok2 = tok; tok2; tok2 = tok2->next()) {
                if (tok2->str() == MatchCompiler::makeConstString(";"))
                    break;
                if (match37(tok2, varid)) {
                    used = true;
                    break;
                }
                if (match38(tok2, varid)) {
                    used = true;
                    break;
                }
            }
