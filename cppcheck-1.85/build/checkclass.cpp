...
// pattern: %var% = new|malloc|g_malloc|g_try_malloc|realloc|g_realloc|g_try_realloc
static bool match10(const Token* tok) {
    ...
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("=")))
        return false;
    tok = tok->next();
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("new")) || (tok->str()==MatchCompiler::makeConstString("malloc")) || (tok->str()==MatchCompiler::makeConstString("g_malloc")) || (tok->str()==MatchCompiler::makeConstString("g_try_malloc")) || (tok->str()==MatchCompiler::makeConstString("realloc")) || (tok->str()==MatchCompiler::makeConstString("g_realloc")) || (tok->str()==MatchCompiler::makeConstString("g_try_realloc"))))
        return false;
    return true;
}
...
static bool match45(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("try")) || (tok->str()==MatchCompiler::makeConstString("do"))))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("{")))
        return false;
...
// pattern: %varid% =|(|{
template<class T> static T * findmatch48(T * start_tok, unsigned int varid) {
...
// pattern: ; __property ;
template<class T> static T * findmatch49(T * start_tok, const Token * end) {
...
static bool match56(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("&")) || (tok->str()==MatchCompiler::makeConstString("*")) || (tok->varId() != 0)))
        return false;
    return true;
}
// pattern: *
...
// pattern: %var% = calloc|malloc|realloc|g_malloc|g_try_malloc|g_realloc|g_try_realloc (
static bool match58(const Token* tok) {
    ...
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("=")))
        return false;
    tok = tok->next();
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("calloc")) || (tok->str()==MatchCompiler::makeConstString("malloc")) || (tok->str()==MatchCompiler::makeConstString("realloc")) || (tok->str()==MatchCompiler::makeConstString("g_malloc")) || (tok->str()==MatchCompiler::makeConstString("g_try_malloc")) || (tok->str()==MatchCompiler::makeConstString("g_realloc")) || (tok->str()==MatchCompiler::makeConstString("g_try_realloc"))))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("(")))
        return false;
...
// pattern: %var% = malloc|realloc|calloc|new
static bool match64(const Token* tok) {
    ...
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("=")))
        return false;
    tok = tok->next();
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("malloc")) || (tok->str()==MatchCompiler::makeConstString("realloc")) || (tok->str()==MatchCompiler::makeConstString("calloc")) || (tok->str()==MatchCompiler::makeConstString("new"))))
        return false;
    return true;
}
// pattern: free ( %var%
...
// pattern: this - %name%
template<class T> static T * findmatch74(T * start_tok) {
    for (; start_tok; start_tok = start_tok->next()) {
...
static bool match78(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString(";")) || (tok->str()==MatchCompiler::makeConstString("{")) || (tok->str()==MatchCompiler::makeConstString("}")) || (tok->str()==MatchCompiler::makeConstString("public:")) || (tok->str()==MatchCompiler::makeConstString("private:")) || (tok->str()==MatchCompiler::makeConstString("protected:"))))
        return false;
    return true;
}
...
static bool match79(const Token* tok) {
    ...
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("::")))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("shared_ptr")))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("<")))
        return false;
...
// pattern: return|if|string|switch|while|catch|for
static bool match87(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("return")) || (tok->str()==MatchCompiler::makeConstString("if")) || (tok->str()==MatchCompiler::makeConstString("string")) || (tok->str()==MatchCompiler::makeConstString("switch")) || (tok->str()==MatchCompiler::makeConstString("while")) || (tok->str()==MatchCompiler::makeConstString("catch")) || (tok->str()==MatchCompiler::makeConstString("for"))))
        return false;
    return true;
}
// pattern: ( ) (
...
static bool match93(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("if")) || (tok->str()==MatchCompiler::makeConstString("switch"))))
        return false;
    return true;
}
// pattern: ) {
...
// pattern: if|switch|while|for|do|}
static bool match98(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("if")) || (tok->str()==MatchCompiler::makeConstString("switch")) || (tok->str()==MatchCompiler::makeConstString("while")) || (tok->str()==MatchCompiler::makeConstString("for")) || (tok->str()==MatchCompiler::makeConstString("do")) || (tok->str()==MatchCompiler::makeConstString("}"))))
        return false;
    return true;
}
/*
    ...
    for (const Scope * scope : mSymbolDatabase->classAndStructScopes) {
        ...
        if (scope->numConstructors == 0 && printStyle && !usedInUnion) {
            ...
            for (const Variable &var : scope->varlist) {
                if (var.isPrivate() && !var.isStatic() && !match1(var.nameToken(), var.declarationId()) &&
    ...
    for (const Scope * scope : mSymbolDatabase->classAndStructScopes) {
        ...
        for (const Function &func : scope->functionList) {
            ...
            for (const Token* const end = func.functionScope->bodyStart; tok != end; tok = tok->next()) {
                if (match2(tok) ||
                    (match3(tok) && mSettings->library.alloc(tok->tokAt(2)))) {
            ...
            for (const Token* const end = func.functionScope->bodyEnd; tok != end; tok = tok->next()) {
                if (match4(tok) ||
                    (match5(tok) && mSettings->library.alloc(tok->tokAt(2)))) {
                    const Variable* var = tok->variable();
        ...
        for (const Function &func : scope->functionList) {
            ...
            if (tok->str()==MatchCompiler::makeConstString(":")) {
                ...
                while (match7(tok)) {
                    if (allocatedVars.find(tok->varId()) != allocatedVars.end()) {
                        if (tok->varId() && match8(tok->tokAt(2)))
                            copiedVars.insert(tok);
                        else if (!match9(tok->tokAt(2)))
            ...
            for (tok = func.functionScope->bodyStart; tok != func.functionScope->bodyEnd; tok = tok->next()) {
                ...
                } else if (match11(tok) && allocatedVars.find(tok->varId()) != allocatedVars.end()) {
                    copiedVars.insert(tok);
                }
            }
            break;
...
void CheckClass::initializeVarList(const Function &func, std::list<const Function *> &callstack, const Scope *scope, std::vector<Usage> &usage)
{
    ...
    for (; ftok && ftok != func.functionScope->bodyEnd; ftok = ftok->next()) {
        ...
        if (initList) {
            if (level == 0 && match12(ftok) && match13(ftok->linkAt(1))) {
            ...
            } else if (level != 0 && match14(ftok)) // assignment in the initializer: var(value = x)
                assignVar(ftok->varId(), scope, usage);

            // Level handling
            if (ftok->link() && match15(ftok))
                level++;
            else if (ftok->str() == MatchCompiler::makeConstString("{")) {
                if (level != 0 ||
                    (match16(ftok->previous()) && match17(ftok->link())))
                    level++;
                else
                    initList = false;
            } else if (ftok->link() && match18(ftok))
                level--;
        }
        ...
        if (match19(ftok) && isLikelyStreamRead(true, ftok)) {
        ...
        if (match22(ftok->next())) {
            assignAllVar(usage);
            break;
        }

        // Using swap to assign all variables..
        if (func.type == Function::eOperatorEqual && match23(ftok) && match24(ftok->linkAt(2))) {
            assignAllVar(usage);
            break;
        }
        ...
        else if (match37(ftok) &&
                 ftok->previous()->str() != MatchCompiler::makeConstString("::")) {
        ...
        } else if (match38(ftok) && ftok->str() != MatchCompiler::makeConstString("if")) {
            if (ftok->str() == MatchCompiler::makeConstString("::"))
                ftok = ftok->next();
        ...
        // Assignment of array item of member variable?
        else if (match28(ftok)) {
            assignVar(ftok->next()->varId(), scope, usage);
        } else if (match43(ftok)) {
...
void CheckClass::initializationListUsage()
{
    ...
    for (const Scope *scope : mSymbolDatabase->functionScopes) {
        ...
        for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            if (match7(tok)) // Assignments might depend on this function call or if/for/while/switch statement from now on.
                break;
            if (match45(tok))
                break;
            if (match46(tok) && tok->strAt(-1) != MatchCompiler::makeConstString("*")) {
                const Variable* var = tok->variable();
                if (var && var->scope() == owner && !var->isStatic()) {
                    ...
                    for (const Token* tok2 = tok->tokAt(2); tok2 && tok2->str() != MatchCompiler::makeConstString(";"); tok2 = tok2->next()) {
                        ...
                        if (var2) {
                            ...
                            allowed = false;
                            break;
                        } else if (match7(tok2) && tok2->strAt(-1) != MatchCompiler::makeConstString(".") && isMemberFunc(owner, tok2)) { // Member function called?
                            allowed = false;
                            break;
                        }
                    }
...
static bool checkFunctionUsage(const Function *privfunc, const Scope* scope)
{
    ...
    for (const Variable &var : scope->varlist) {
        if (var.isStatic()) {
            const Token* tok = findmatch48(scope->bodyEnd, var.declarationId()) ;
            if (tok)
                tok = tok->tokAt(2);
...
void CheckClass::privateFunctions()
{
    ...
    for (const Scope * scope : mSymbolDatabase->classAndStructScopes) {
        ...
        if (findmatch49(scope->bodyStart, scope->bodyEnd) )
            continue;
    ...
    for (const Scope *scope : mSymbolDatabase->functionScopes) {
        for (const Token *tok = scope->bodyStart; tok && tok != scope->bodyEnd; tok = tok->next()) {
            if (match50(tok)) {
                ...
                else if (match52(arg3))
                    typeTok = arg3->tokAt(4);
                else if (match53(arg3))
                    typeTok = arg3->tokAt(3);
                else if (match54(arg3) || match55(arg1)) {
                    type = findFunctionOf(arg3->scope());
                } else if (match56(arg1)) {
            ...
            } else if (tok->variable() && tok->variable()->typeScope() && match58(tok)) {
                const std::set<const Scope *> parsedTypes;
...
void CheckClass::checkMemsetType(const Scope *start, const Token *tok, const Scope *type, bool allocation, std::set<const Scope *> parsedTypes)
{
    ...
    for (const Variable &var : type->varlist) {
        ...
        if (!var.isStatic() && !var.isConst() && !var.isPointer() && (!var.isArray() || var.typeEndToken()->str() != MatchCompiler::makeConstString("*"))) {
            ...
            // check for std:: type
            if (var.isStlType() && tok1->strAt(2) != MatchCompiler::makeConstString("array") && !mSettings->library.podtype(tok1->strAt(2)))
                if (allocation)
        ...
        for (std::list<Function>::const_iterator func = scope->functionList.begin(); func != scope->functionList.end(); ++func) {
            if (func->type == Function::eOperatorEqual && func->access == Public) {
                ...
                if (func->retDef->str() == scope->className) {
                    if (match59(func->retDef)) {
                        returnSelfRef = true;
                    } else {
                        // We might have "Self<template_parameters>&""
                        const Token * const tok = func->retDef->next();
                        if (tok && tok->str() == MatchCompiler::makeConstString("<") && tok->link() && tok->link()->next() && tok->link()->next()->str() == MatchCompiler::makeConstString("&"))
                            returnSelfRef = true;
                    }
                }
                if (!returnSelfRef) {
                    ...
                    if (match60(paramTok)) {
                        if (paramTok->str() == MatchCompiler::makeConstString("const") &&
    ...
    for (const Scope * scope : mSymbolDatabase->classAndStructScopes) {
        for (std::list<Function>::const_iterator func = scope->functionList.begin(); func != scope->functionList.end(); ++func) {
            if (func->type == Function::eOperatorEqual && func->hasBody()) {
                ...
                if (match59(func->retDef) && func->retDef->str() == scope->className) {
...
void CheckClass::checkReturnPtrThis(const Scope *scope, const Function *func, const Token *tok, const Token *last, std::set<const Function*>& analyzedFunctions)
{
    ...
    for (; tok && tok != last; tok = tok->next()) {
        ...
        if (tok->strAt(2) == MatchCompiler::makeConstString("(") &&
            tok->linkAt(2)->next()->str() == MatchCompiler::makeConstString(";")) {
            ...
            for (std::list<Function>::const_iterator it = scope->functionList.begin(); it != scope->functionList.end(); ++it) {
                ...
                if (it->type == Function::eFunction && it->hasBody() &&
                    ...
                    if (it->tokenDef->previous()->str() == MatchCompiler::makeConstString("&") &&
...
bool CheckClass::hasAllocation(const Function *func, const Scope* scope) const
{
    ...
    for (const Token *tok = func->functionScope->bodyStart; tok && (tok != last); tok = tok->next()) {
        if (match64(tok) && isMemberVar(scope, tok))
            return true;
...
bool CheckClass::hasAssignSelf(const Function *func, const Token *rhs)
{
    ...
    for (const Token *tok = func->functionScope->bodyStart; tok && tok != last; tok = tok->next()) {
        ...
        while (!tokens.empty()) {
            ...
            if (!match70(tok2))
                continue;
            if (match71(tok2->astOperand1()))
                tok2 = tok2->astOperand2();
            else if (match71(tok2->astOperand2()))
...
void CheckClass::checkConst()
{
    ...
    for (const Scope * scope : mSymbolDatabase->classAndStructScopes) {
        for (const Function &func : scope->functionList) {
            ...
            if (match75(previous)) {
                if (func.retDef->str() != MatchCompiler::makeConstString("const"))
                    continue;
            } else if (match76(previous->previous())) {
                const Token *temp = previous->previous();

                bool foundConst = false;
                while (!match77(temp->previous())) {
                    temp = temp->previous();
                    if (temp->str() == MatchCompiler::makeConstString("const")) {
                        foundConst = true;
                        break;
                    }
                }
                ...
                if (!foundConst)
                    continue;
            } else if (func.isOperator() && match78(previous)) { // Operator without return type: conversion operator
                const std::string& opName = func.tokenDef->str();
                if (opName.compare(8, 5, "const") != 0 && (endsWith(opName,'&') || endsWith(opName,'*')))
                    continue;
            } else if (match79(func.retDef)) {
...
bool CheckClass::isMemberVar(const Scope *scope, const Token *tok) const
{
    ...
    do {
        ...
        if (tok->str() == MatchCompiler::makeConstString("this")) {
            return true;
        } else if (match80(tok->tokAt(-3))) {
            return true;
        } else if (match81(tok->tokAt(-2))) {
            tok = tok->tokAt(-2);
            again = true;
        } else if (match82(tok->tokAt(-2))) {
    ...
    if (!tok->function()) {
        for (const Function &func : scope->functionList) {
            if (func.name() == tok->str()) {
                ...
                size_t argsPassed = tok2->str() == MatchCompiler::makeConstString(")") ? 0 : 1;
                for (;;) {
...
bool CheckClass::checkConstFunc(const Scope *scope, const Function *func, bool& memberAccessed) const
{
    ...
    for (const Token *tok1 = func->functionScope->bodyStart; tok1 && tok1 != func->functionScope->bodyEnd; tok1 = tok1->next()) {
        if (tok1->isName() && isMemberVar(scope, tok1)) {
            ...
            if (tok1->str() == MatchCompiler::makeConstString("this") && tok1->previous()->isAssignmentOp())
                return false;
            ...
            if (lhs->str() == MatchCompiler::makeConstString("&")) {
                ...
                if (lhs->tokType() == Token::eAssignmentOp && lhs->previous()->variable()) {
                    if (lhs->previous()->variable()->typeStartToken()->strAt(-1) != MatchCompiler::makeConstString("const") && lhs->previous()->variable()->isPointer())
                        return false;
                }
            } else if (lhs->str() == MatchCompiler::makeConstString(":") && lhs->astParent() && lhs->astParent()->str() == MatchCompiler::makeConstString("(") && tok1->strAt(1) == MatchCompiler::makeConstString(")")) { // range-based for-loop (C++11)
        ...
        // streaming: >> *this
        else if (match86(tok1) && isLikelyStreamRead(true, tok1)) {
            return false;
        }

        // function call..
        else if (match7(tok1) && !tok1->isStandardType() &&
                 !match87(tok1)) {
            if (isMemberFunc(scope, tok1) && tok1->strAt(-1) != MatchCompiler::makeConstString(".")) {
        ...
        } else if (match90(tok1) && (!tok1->link() || !match89(tok1->link()->previous()))) {
            return false;
        }
    }
...
void CheckClass::checkSelfInitialization()
{
    for (const Scope *scope : mSymbolDatabase->functionScopes) {
        ...
        for (; tok != scope->bodyStart; tok = tok->next()) {
            if (match92(tok) && tok->next()->varId() == tok->tokAt(3)->varId()) {
        ...
        std::map<const Function *, std::list<const Token *> > & virtualFunctionCallsMap)
{
    ...
    for (const Token *tok = function.arg->link(); tok != function.functionScope->bodyEnd; tok = tok->next()) {
        if (function.type != Function::eConstructor &&
            ...
            if ((match94(tok) && tok->link() && match93(tok->link()->previous())) ||
                match95(tok)) {
        ...
        if (callFunction->isVirtual()) {
            if (!callFunction->isPure() && match96(tok->previous()))
                continue;
    ...
    const std::string &funcname)
{
    ...
    if (scopeFunction) {
        ...
        for (const Token *tok = scopeFunction->tokenDef; tok != endToken; tok = tok->next()) {
            if (!constructorName.empty() && match97(tok->previous()))
                constructorName += ' ';
