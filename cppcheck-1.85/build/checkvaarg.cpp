...
static bool match2(const Token* tok, const unsigned int varid) {
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("va_start")))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str()==MatchCompiler::makeConstString("(")))
...
static bool match5(const Token* tok) {
    if (!tok || !((tok->str()==MatchCompiler::makeConstString("throw")) || (tok->str()==MatchCompiler::makeConstString("return"))))
        return false;
    return true;
}
/*
    ...
    for (const Variable* var : symbolDatabase->variableList()) {
        ...
        for (;  tok && tok != var->scope()->bodyEnd; tok = tok->next()) {
            if (match2(tok, var->declarationId())) {
                ...
                if (!tok)
                    return;
            } else if (tok->str() == MatchCompiler::makeConstString("goto") || (mTokenizer->isCPP() && tok->str() == MatchCompiler::makeConstString("try"))) {
                open = false;
                break;
